...
@org.springframework.stereotype.Service("distroConsistencyService") /* AP - 内存注册数据 */
public class DistroConsistencyServiceImpl implements EphemeralConsistencyService, DistroDataProcessor {

    private final DistroMapper distroMapper;

    private final DataStore dataStore;/* 内存数据库 */
    ...
    public void onPut(String key, Record value) {
        ...
        if (KeyBuilder.matchEphemeralInstanceListKey(key)) {
            ...
            dataStore.put(key, datum); /* 添加服务实例列表 到内存数据库 */
        }
        ...
        notifier.addTask(key, DataOperation.CHANGE); /* 服务实例变更异步任务 */
    }
        ...
        private void handle(Pair<String, DataOperation> pair) {
            try {
                ...
                for (RecordListener listener : listeners.get(datumKey)) {
                    ...
                    try {
                        if (action == DataOperation.CHANGE) {
                            listener.onChange(datumKey, dataStore.get(datumKey).value); /* ## 更新服务列表 */
                            continue;
                        }
